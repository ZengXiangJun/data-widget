...
### 分层显示【新增】
组件支持分层显示，需在数据源中新增 `widget_floor` 字段：
```js
[{                                 
    "key": "value",
    "widget_floor": 1
...
情况说明：
1. 若开启分层显示，组件会读取数据源中 `widget_floor` 字段，若 `!widget_floor`，默认此行数据为 0。
2. 组件会将 `widget_floor` 字段值相同的单元渲染在同一行，不换行。
3. 默认为升序分层，即按照 `widget_floor` 字段值从小到大的顺序渲染层级。
